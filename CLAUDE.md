...
| `assets/js/scoreboard.js` | Fetches scores from Apps Script, parses and renders a ranked table into `#scoreboard-body`/`#scoreboard-status`/`#last-updated`. **Currently orphaned and broken**: every page's nav (including live pages) links to `scoreboard.html`, but that file does not exist anywhere in the repo, and no `.html` file includes this script — the nav "Scoreboard" link 404s site-wide. Fixing it means creating `scoreboard.html` with those three element IDs and a `<script src="assets/js/scoreboard.js">` tag |
